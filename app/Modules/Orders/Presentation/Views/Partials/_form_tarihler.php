...
<div class="form-section sec-tarih mt">
    ...
    <div class="g-auto g-tarih">
        <div><label>Sipariş Tarihi</label><input type="date" name="siparis_tarihi" class="form-control" value="<?= h(safe_date_val($order['siparis_tarihi'] ?? '') ?: date('Y-m-d')) ?>" <?= (!empty($__readonly) || !empty($__uretim_readonly)) ? 'readonly style="background:#f8fafc;opacity:.8"' : '' ?>></div>
...
<style>
  .sec-tarih input { pointer-events:none; opacity:.8; background:#f8fafc !important; }
</style>
<?php endif; ?>
</div>
<script>
