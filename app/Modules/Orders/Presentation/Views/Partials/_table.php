              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2px; width: 100%;">
                ...
                <?php if ($___role !== 'musteri'): ?>
                  <div style="grid-column: 1; width:100%;">
                    ...
                    if ($___show_delete):
                      if ($___is_admin): ?>
                        <a class="btn" href="order_delete.php?id=<?= $o->id ?>&confirm=EVET" onclick="return confirm('Silmek istediğinize emin misiniz?');" title="Sil" style="width:100%; height:30px; padding:0; display:flex; align-items:center; justify-content:center; border-radius:15px; background:#fff; border:1px solid #e1e5eaff; color:#ef4444;"><span style="font-size:15px;">🗑️</span></a>
                      <?php else:
                        $___tm = sprintf('%d:%02d', floor($___remaining_sec / 60), $___remaining_sec % 60); ?>
                        <a class="btn btn-delete-timer" href="order_delete.php?id=<?= $o->id ?>&confirm=EVET" data-remaining="<?= (int)$___remaining_sec ?>" onclick="return confirm('Silmek istiyor musunuz?');" style="width:100%; height:30px; padding:0; display:flex; align-items:center; justify-content:center; border-radius:15px; font-size:9px; --timer-pct:<?= number_format($___remaining_pct, 2) ?>%"><?= $___tm ?></a>
                      <?php endif; ?>
                    <?php endif; ?>
                  </div>
                  <?php if (in_array($___role, ['admin', 'sistem_yoneticisi'], true)): ?>
                    <div style="grid-column: 2; width:100%;">
                      <a class="btn" href="order_send_mail.php?id=<?= $o->id ?>" title="Mail Gönder" style="width:100%; height:30px; padding:0; display:flex; align-items:center; justify-content:center; border-radius:15px; background:#fff; border:1px solid #e1e5eaff; color:#d97706;"><span style="font-size:15px;">📧</span></a>
                    </div>
                  <?php endif; ?>
                <?php endif; ?>
              </div>
